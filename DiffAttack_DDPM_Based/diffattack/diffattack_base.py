    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        for _ in range(self.eot_iter):
            if not self.is_tf_model:
                ...
                if self.model.runner.total_noise_levels == 0:
                    loss_, grad_all = self.model.runner.compute_efficient_gradient_classifier(self.model.classifier, criterion_indiv, mid_x[-1], y)
                else:
                    loss_, grad_ = self.model.runner.compute_efficient_gradient_classifier(self.model.classifier, criterion_indiv, mid_x[-1], y)

                    for idx_ in reversed(list(range(0, self.model.runner.total_noise_levels))):

                        if idx_ in target_ind:
                            grad_ = self.model.runner.compute_efficient_gradient_mse(grad_, idx_)
                        else:
                            grad_ = self.model.runner.compute_efficient_gradient(grad_, idx_)

                    grad_all = self.model.runner.compute_efficient_gradient_diffusion_process(x_adv, grad_)
        ...
        for i in range(self.n_iter):
            ...
            for _ in range(self.eot_iter):
                if not self.is_tf_model:
                    ...
                    if self.model.runner.total_noise_levels == 0:
                        loss_, grad_all = self.model.runner.compute_efficient_gradient_classifier(
                            self.model.classifier, criterion_indiv, mid_x[-1], y)
                    else:
                        loss_, grad_ = self.model.runner.compute_efficient_gradient_classifier(self.model.classifier,criterion_indiv, mid_x[-1], y)

                        norm_ori = torch.norm(grad_, p=2)
