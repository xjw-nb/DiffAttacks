    ...
    def rvpsde_fn(self, t, x, return_type='drift'):
        ...
        if return_type == 'drift':
            ...
            if self.score_type == 'guided_diffusion':
                ...
                if self.store_score_or_not:
                    print(self.img_shape)
                    if self.img_shape[1]==256:
                        # imagenet
                        self.score_store[150 - int(t[0].item() * 1000)] = score
                    elif self.img_shape[1]==32:
                        # cifar
                        self.score_store[100 - int(t[0].item() * 1000)] = score
                    else:
                        assert 1==0, 'error'
...
class RevGuidedDiffusion(torch.nn.Module):
    def __init__(self, args, config, device=None):
        ...
        if config.data.dataset == 'ImageNet':
            ...
            model.load_state_dict(torch.load(f'./pretrained/256x256_diffusion_uncond.pt', map_location='cpu'))
    ...
    def image_editing_sample(self, img, bs_id=0, tag=None, return_std_z=False):
        ...
        for it in range(self.args.sample_step):
            ...
            t_size = self.args.t
            ts = torch.linspace(t0, t1, t_size+1).to(self.device)

            x_ = x.view(batch_size, -1)  # (batch_size, state_size)
            xs__ = torchsde.sdeint_adjoint(self.rev_vpsde, x_, ts, method='euler')
            mid_x = []
            for i in range(1,xs__.shape[0]):
                mid_x.append(xs__[i])


            x0 = mid_x[-1].view(x.shape)  # (batch_size, c, h, w)
        ...
        if return_std_z:
            return torch.cat(xs, dim=0), mid_x, ori_x, a_std, z__
